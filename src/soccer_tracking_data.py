...
def get_world_cup_matches() -> Dict[str, int]:
    """
    ...
    -------
    Dict[str, int]
        Dictionary mapping match names (e.g., "Argentina vs France") to match IDs
    
    Examples
    --------
    >>> matches = get_world_cup_matches()
    >>> print(matches)
    {'Argentina vs France': 3869151, ...}
    """
    try:
        ...
        # Create dictionary mapping match names to match IDs
        match_dict = {}
        for _, match in matches_df.iterrows():
            ...
            # Create readable match name
            match_name = f"{home_team} vs {away_team}"
            if match_date:
                match_name = f"{match_name} ({match_date})"
            
            match_dict[match_name] = match_id
        ...
        return {"Argentina vs France (2022-12-18)": 3869151}
        ...
        try:
            ...
            for player_data in freeze_frame:
                try:
                    ...
                    if team_name is True:
                        team_name = shot.get('team', 'Teammate')
                    elif team_name is False:
                        team_name = 'Opponent'
                    else:
                        team_name = 'Unknown'
